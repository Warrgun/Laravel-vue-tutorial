...
const submit = () =>{
  axiosClient.get('/sanctum/csrf-cookie').then(response => {
    axiosClient.post('/login', data.value).then( response => router.push({name:'Home'}))
  });
}
</script>
<template>
    <GuestLayout>
        ...
        <p class="mt-10 text-center text-sm/6 text-gray-500">
          Not a member?
          {{ ' ' }}
          <RouterLink :to="('SignUp')" class="font-semibold text-indigo-600 hover:text-indigo-500">Create an account</RouterLink>
        </p>
      </div>
    </GuestLayout>
</template>
